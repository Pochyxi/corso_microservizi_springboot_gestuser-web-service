...
public class UtentiControllerTest 
{
	...
	{
		mockMvc.perform(MockMvcRequestBuilders.get("/api/utenti/cerca/userid/Adiener")
				.accept(MediaType.APPLICATION_JSON))
				...
				.andExpect(jsonPath("$.userId").value("Adiener"))
				...
				assertThat(passwordEncoder.matches("123_Stella", 
						utentiRepository.findByUserId("Adiener").getPassword()))
				.isEqualTo(true);
	}
	...
	public void testGetAllUser() throws Exception
	{
		mockMvc.perform(MockMvcRequestBuilders.get("/api/utenti/cerca/tutti")
				...
				.andExpect(jsonPath("$[0].userId").value("Adiener"))
	...
	{
		mockMvc.perform(MockMvcRequestBuilders.delete("/api/utenti/elimina/Adiener")
				.accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.code").value("200 OK"))
				.andExpect(jsonPath("$.message").value("Eliminazione Utente Adiener Eseguita Con Successo"))
				.andDo(print());
	}
